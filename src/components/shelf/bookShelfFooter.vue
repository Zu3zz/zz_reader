  ...
  <div class="book-shelf-footer">
    <div class="book-shelf-tab-wrapper" v-for="(item, index) in tabs" :key="index" @click="onTabClick(item)">
      <div class="book-shelf-tab">
        ...
        <div class="icon-private-see tab-icon" :class="{'is-selected': isSelected}" v-if="item.index === 1 && isPrivate"></div>
    ...
    <popup ref="popup" :title="popTitle" :confirmText="confirmText" :isRemoveText="isRemoveText" :cancelText="$t('shelf.cancel')" @confirm="onConfirm"></popup>
    <shelf-group-dialog :visible.sync="ifGroupDialogShow" :data="data" :bookList="bookList" :category="category" :isInGroup="isInGroup" @group="groupEdit"></shelf-group-dialog>
  </div>
</template>
